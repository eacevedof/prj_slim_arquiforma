<?php

require_once __DIR__ . "/Bootstrap/functions.php";
require_once __DIR__ . "/Bootstrap/ConstantEnum.php";
require_once __DIR__ . "/Bootstrap/VariablesEntity.php";

require_once __DIR__ . "/Http/HttpRequest.php";
require_once __DIR__ . "/Enums/XmlAttributeEnum.php";
require_once __DIR__ . "/Enums/XmlTagEnum.php";

require_once __DIR__ . "/Xml/HtmlOutput.php";
require_once __DIR__ . "/Xml/XmlInput.php";
require_once __DIR__ . "/Xml/XmlOutput.php";
require_once __DIR__ . "/Xml/XmlInterpreter.php";
require_once __DIR__ . "/Xml/XmlSql.php";

require_once __DIR__ . "/Checkers/PhpExtensions.php";
require_once __DIR__ . "/Checkers/Php.php";
require_once __DIR__ . "/Checkers/Mysql.php";

require_once __DIR__ . "/PDO/PdoMysql.php";
